...
class NinchatSendRatingsTest {
    ...
    fun update_member_with_empty_session() = runBlocking<Unit> {
        val actionId = NinchatSendRatings.execute(null)
    ...
    @Test
    fun update_member_with_no_message() = runBlocking<Unit> {
        val sessionChannel = createSession()
        repeat(1) {
            val currentSession = sessionChannel.receive()
            val requestAudience = Channel<Boolean>()
            ...
            repeat(1) {
                val hasError = requestAudience.receive()
                Assert.assertEquals("should failed with empty message", true, hasError)
            }
        }
    }

    @Test
    fun update_member_with_wrong_channel_id_and_message_payload() = runBlocking<Unit> {
        val sessionChannel = createSession()
        repeat(1) {
            val currentSession = sessionChannel.receive()
            val requestAudience = Channel<Boolean>()
